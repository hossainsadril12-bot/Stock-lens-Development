...
const transfers: (string | number | null)[][] = [
  // code, item_id, item_name, qty, from, to, status, vehicle, driver, phone, dispatched, expected, received, by
  ["TR-2001", 2, "A4 Standard Paper", 100, 1, 4, "in_transit", "Truck DHA-11-2345", "Karim Ali", "+8801711-000111", daysAgo(2), "2026-08-01", null, "Anwar"],
  ["TR-2000", 6, "Packaging Box (Large)", 300, 1, 3, "received", "Pickup CTG-3-9911", "Jamal Uddin", "+8801822-333444", daysAgo(9), daysAgo(7), daysAgo(7), "Anwar"],
];
for (const t of transfers)
  add(
    "INSERT INTO transfer_orders (code,item_id,item_name,quantity,from_location_id,to_location_id,status,vehicle,driver_name,driver_phone,dispatched_at,expected_date,received_at,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    t
  );
...
async function main() {
  ...
  console.log(`Seeded ${demoUsers.length} users, ${itemRows.length} items, ${stockRows.length} stock rows, ${moves.length} movements, 3 POs, ${transfers.length} transfers into ${url}`);
}
